...
def process_empathetic_dialogues(csv_path: str, out_dir: str):
    ...
    CSV columns: index, Situation, emotion, empathetic_dialogues, labels, , 
    empathetic_dialogues contains the conversation history.
    labels contains the next utterance.
    We extract seeker (Customer) utterances as input, supporter (Agent) as target.
    """
    ...
    with open(csv_path, encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            if len(row) < 5:
                continue
            situation = row[1].strip()
            emotion = row[2].strip().lower()
            empathetic_dialogues = row[3].strip()
            labels = row[4].strip()
            
            if not empathetic_dialogues or not labels:
                continue
            
            # Parse the last Customer utterance from empathetic_dialogues
            lines = empathetic_dialogues.split('\n')
            seeker_utt = None
            for line in reversed(lines):
                line = line.strip()
                if line.startswith('Customer :'):
                    seeker_utt = line[len('Customer :'):].strip()
                    break
            
            if not seeker_utt or len(seeker_utt.split()) < 3:
                continue
            
            # labels is the Agent's response
            supporter_utt = labels
            if len(supporter_utt.split()) < 3:
                continue
    ...
    print(f"[ED] Total conversations: N/A (processed {len(pairs)} pairs)")
