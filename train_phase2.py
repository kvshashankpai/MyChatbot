...
Key differences from Phase 1:
  ...
  - Early stop on composite score, NOT raw Distinct-2

FIXES vs original:
  1. Early stopping metric changed from raw D2 → composite score:
         score = 0.5 * D2 + 0.3 * emo_f1 + 0.2 * str_f1
     Raw D2 never improved past epoch 2 (0.021) on the small ESConv set,
     so the model stopped with a completely undertrained checkpoint.
     The composite score is stable even when D2 plateaus.

  2. Default patience raised from 8 → 15. ESConv has only ~1.6k training
     samples; the composite metric is noisy and needs more runway.

  3. Default epochs raised from 40 → 50 to give the composite metric space.

  4. Layer-wise LR decay: encoder layers get 0.5× the decoder LR so the
     pre-trained language model representations aren't overwritten by the
     small ESConv dataset.

  5. Beam search seed in evaluation now includes the strategy token, matching
     the training decoder_input format exactly.

  6. Validation greedy generation uses strategy token injection to get
     coherent Distinct-N scores that reflect actual inference quality.
"""
...
from utils import (
    ...
    build_optimizer_with_layer_decay,
    compute_perplexity,
    compute_distinct_n,
...
def parse_args():
    ...
    p.add_argument("--epochs",            type=int,   default=50)   # FIX: was 40
    p.add_argument("--batch_size",        type=int,   default=16)
    p.add_argument("--grad_accum",        type=int,   default=8)
    p.add_argument("--peak_lr",           type=float, default=2e-5)
    p.add_argument("--encoder_lr_decay",  type=float, default=0.5,
                   help="Encoder LR = peak_lr * this factor (prevents forgetting)")
    ...
    p.add_argument("--patience",          type=int,   default=15)   # FIX: was 8
    p.add_argument("--use_wandb",         action="store_true")
    return p.parse_args()


def composite_score(val: dict) -> float:
    """
    FIX: Composite early-stopping metric.

    Raw D2 collapses on tiny datasets (ESConv ~1.6k samples).
    A composite of diversity + classification F1 is more stable
    and reflects overall model quality better than D2 alone.

    Weights:
      0.50 × Distinct-2        (generation diversity)
      0.30 × emotion_f1        (NLU quality)
      0.20 × strategy_f1       (strategy routing quality)
    """
    return (
        0.50 * val.get("distinct2",   0.0)
        + 0.30 * val.get("emotion_f1",  0.0)
        + 0.20 * val.get("strategy_f1", 0.0)
    )
...
def evaluate(model, loader, loss_fn, tokenizer, device, max_gen=200):
    ...
    # Build strategy token id lookup for seed injection
    strat_token_ids = {}
    for i, s in enumerate(CANONICAL_STRATEGIES):
        tid = tokenizer.token_to_id(f"[strategy_{s.upper()}]")
        strat_token_ids[i] = tid  # None if not in vocab
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # FIX: greedy generation with strategy token seed injection,
            # matching the training decoder_input format.
            if len(gen_seqs) < max_gen:
                enc  = model.encode(src)
                mem  = enc["memory"]
                smk  = enc["src_mask"]
                pred_strat_ids = enc["strategy_logits"].argmax(-1).cpu().tolist()

                for b_idx in range(src.size(0)):
                    # Build per-sample seed: [BOS] + optional strategy token
                    strat_id   = pred_strat_ids[b_idx]
                    strat_tid  = strat_token_ids.get(strat_id)
                    seed_ids   = [bos] + ([strat_tid] if strat_tid is not None else [])
                    cur = torch.tensor([seed_ids], dtype=torch.long, device=device)
                    mem_b = mem[b_idx:b_idx+1]
                    smk_b = smk[b_idx:b_idx+1]

                    for _ in range(64):
                        nxt = model.decode_step(cur, mem_b, smk_b).argmax(-1, keepdim=True)
...
def main():
    ...
    # FIX: layer-wise LR decay — encoder at lower LR to prevent forgetting
    optimizer = build_optimizer_with_layer_decay(
        model, args.peak_lr, args.encoder_lr_decay, args.weight_decay
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        score = composite_score(val)

        print(
            f"[Ep {epoch:2d}] ppl={val['perplexity']:.1f}"
            f"  D1={val['distinct1']:.3f}  D2={val['distinct2']:.3f}"
            f"  emo_f1={val['emotion_f1']:.3f}  str_f1={val['strategy_f1']:.3f}"
            f"  score={score:.4f}  tf={tf_ratio:.2f}"
        )

        if args.use_wandb:
            import wandb
            wandb.log({
                **{"epoch": epoch, "tf_ratio": tf_ratio, "composite_score": score},
        ...
        if score > best_score:
            ...
            patience_c += 1
            print(f"  patience {patience_c}/{args.patience}  best_score={best_score:.4f}")
    ...
    print(f"\n[Phase 2] Done. Best composite score={best_score:.4f}")
